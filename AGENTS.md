...
- **The observing verbs are the same as bulldogger.** `snap` (always on), `frames`, `preflight`, `flt`, `exec`, `probe`. `rerun` joined them on 2026-09-11 (the owner decided it that day) and observes nothing: it runs a command under a clock, because the line depug prints was outliving the sessions that ran it. A function id is `path:name@line:column#k`, where the position is the function's declaration in the TypeScript source and `k` counts entries of that function inside one test window.
...
- **The parser is `typescript`, and depug ships its own copy.** It is the language vendor's parser, pinned to 6.0.3 and carried as a dependency, so the compiler a project is on does not decide whether depug installs. It was a peer dependency at `>=5.5.4 <7.0.0` until 2026-09-03; the measurement that changed it is in `docs/design-decisions.md`. Speed matters less because the transform targets one file or one function per rerun. TypeScript 7 is a separate decision, recorded below.
